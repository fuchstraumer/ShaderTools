...
/** Stage 4. Turns what the compiler said into what the runtime needs.
 *
...
/** The symbols one variant's size expressions may name.
 *
 * `Symbols` holds `std::string_view` values, so the strings it points at must outlive the context.
 * `MakeResolveContext` points at the assignment and the defaults the caller already owns. */
struct ResolveContext
{
    std::vector<SizeSymbol> Symbols;
};

/** Builds the symbol table for one variant.
 *
 * The canonical assignment is used rather than the active one, so every axis is nameable even when a
