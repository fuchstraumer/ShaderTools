...
namespace st {
    ...
    public:
        ...
        uint32_t Compile(const char* name, const char* file_src, const size_t len, const VkShaderStageFlagBits stage);
        uint32_t Compile(const char* path_to_source, const VkShaderStageFlagBits stage = VK_SHADER_STAGE_FLAG_BITS_MAX_ENUM);
