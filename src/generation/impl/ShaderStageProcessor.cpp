...
namespace st
{
    ...
    const std::vector<uint32_t>& ShaderStageProcessor::Compile()
    {
        ...
        if (ftracker.FullSourceStrings.count(stage) == 0)
        {
            std::cerr << "Attempted to compile shader, but full source string has not been generated!";
            if (ftracker.BodyPaths.count(stage) != 0)
            {
                ...
                {
                    std::cerr << "Attempting to generate shader before compiliation failed: " << e.what();
                    throw e;
                }
            }
        }
