    ...
    bool ShaderFileTracker::FindResourceScript(const std::string & fname, const ResourceFile * dest_ptr) {
        ...
        if (fs::exists(fs::path(fname))) {
            ...
            if (ResourceScripts.count(absolute_file_path) != 0) {
                ...
                try {
                    auto iter = ResourceScripts.emplace(absolute_file_path, std::make_unique<ResourceFile>(GetCurrentLuaEnvironment()));
                    ...
                    ScriptFileObservers.emplace_back(std::move(std::make_unique<Observer>(absolute_file_path, delegate_t<void(const std::string&)>::create<ResourceFile, &ResourceFile::Execute>(ptr))));
                    return false;
                }
            }
        }
